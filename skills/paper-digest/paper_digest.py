...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="SoMark 学术论文精读工具：解析论文并提取结构化研究卡片"
    )
    parser.add_argument("-f", "--file", required=True, type=str, help="论文文件路径")
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        default="./paper_digest_output",
        help="输出目录（默认：./paper_digest_output）",
    )
    parser.add_argument(
        "--output-formats",
