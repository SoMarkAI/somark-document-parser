...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="SoMark 招投标文件分析工具：解析招标书并提取关键要素"
    )
    parser.add_argument("-f", "--file", required=True, type=str, help="招标文件路径")
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        default="./tender_analyzer_output",
        help="输出目录（默认：./tender_analyzer_output）",
    )
    parser.add_argument(
        "--output-formats",
